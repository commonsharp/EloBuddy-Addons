...
namespace SharedExperience
{
    class Program
    {
        public static Menu menu = MainMenu.AddMenu("Shared Experience", "m.main");
        public static Menu SettingsMenu = menu.AddSubMenu("Settings", "m.settings");
        public static Menu DrawingsMenu = menu.AddSubMenu("Drawings", "m.drawings");
        ...
        public static void OnGameUpdate(EventArgs args)
        {
            ...
            foreach (AIHeroClient hero in ObjectManager.Get<AIHeroClient>())
            {
                ...
                {
                    Cor[i] = SettingsMenu["invColor"].Cast<ColorPicker>().CurrentValue;//menu.Item("invColor").GetValue<Color>();
        ...
        public static void Drawing_OnDraw(EventArgs args)
        {
            ...
            foreach (AIHeroClient hero in ObjectManager.Get<AIHeroClient>())
            {
                ...
                int x = DrawingsMenu["posX"].Cast<Slider>().CurrentValue;
                int y = DrawingsMenu["posY"].Cast<Slider>().CurrentValue + c * 30;

                if ((Environment.TickCount - TimeChangedExp[i]) < 5000 && InvisibleCount[i] > 0)
                {
                    if (SettingsMenu["drawPredictionCircle"].Cast<CheckBox>().CurrentValue) Drawing.DrawCircle(LastMinionPosition[i], 1500, Color.Red);
                ...
                {
                    if (DrawingsMenu["drawchampionlist"].Cast<CheckBox>().CurrentValue)
                    {
                        Drawing.DrawLine(
                        ...
                        Drawing.DrawText(x+5, y+15, MissingColor[i], hero.ChampionName);
                    }
                }
                else
                {
                ...
                if (!SettingsMenu["showEnemies"].Cast<CheckBox>().CurrentValue) continue;

                int textXOffset = SettingsMenu["positionX"].Cast<Slider>().CurrentValue;
                int textYOffset = SettingsMenu["positionY"].Cast<Slider>().CurrentValue;
                ...
                if (SharingCount[i] > 0)
                {
                    ...
                    if (!SettingsMenu["onlyShowInv"].Cast<CheckBox>().CurrentValue && InvisibleCount[i] == 0)
                    {
        ...
        {
            menu.AddLabel("Credits to AlphaGod (Orignal Source)");
            menu.AddLabel("Credits to coman3 (Color Picker) ");
            menu.AddLabel("Ported by test");
            SettingsMenu.AddGroupLabel("Show");
            SettingsMenu.Add("showEnemies", new CheckBox("Draw Text On Enemy"));
            SettingsMenu.Add("onlyShowInv", new CheckBox("Only Draw Text When Not Visible Enemies"));
            SettingsMenu.Add("drawPredictionCircle", new CheckBox("Draw Prediction Circle"));
            SettingsMenu.AddLabel("Text Color When Not Visible Enemies");
            SettingsMenu.AddColorPicker(new ColorPicker("invColor", Color.FromArgb(255, 245, 25, 25)));
            //SettingsMenu.Add("invColor", "Text Color When Not Visible Enemies").SetValue(Color.FromArgb(255, 245,25,25)));
            SettingsMenu.Add("SetDefaultColor", new CheckBox("Set Color to Default", false)).OnValueChange +=
            delegate (ValueBase<bool> sender, ValueBase<bool>.ValueChangeArgs changeArgs)
            {
                ...
                {
                    menu.Get<ColorPicker>("invColor").SetColor(Color.FromArgb(255, 245, 25, 25)); // Set Colorpickers value
                    Core.DelayAction(() => { sender.CurrentValue = false; }, 125); //DelayAction for Tatical Feedback
                }
            };
            SettingsMenu.Add("positionX", new Slider("OnEnemy Text Position X", 142, -100, 200));
            SettingsMenu.Add("positionY", new Slider("OnEnemy Text Position Y", 21, -100, 100));


            //Champion List Menu
            DrawingsMenu.AddGroupLabel("Champion List");
            DrawingsMenu.Add("drawchampionlist", new CheckBox("Draw Champion List",false));
            DrawingsMenu.Add("posX", new Slider("Champions List Pos X", Drawing.Width / 2, 0, Drawing.Width));
            DrawingsMenu.Add("posY", new Slider("Champions List Pos Y", Drawing.Height / 2, 0, Drawing.Height));
